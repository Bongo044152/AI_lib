...
def test_thinking_invaild():
    ...
    try:
        gpt.chat(ModelIn(content="隨意輸出一些訊息，長度不為0，以測試連線", thinking=True))
    ...
    model_input = ModelIn(content="描述外星人的傳說故事（可自創，但聽起來要足夠真實)", thinking=True)
...
def test_system_prompt():
    print()
    gpt = GPTModel(opt=GPTOption(model="gpt-4o"))
    res = gpt.chat(ModelIn(content="who are you?", system_prompt="不管使用者說什麼，你都回答「很遺憾我沒有辦法幫助到你」，注意！要一字不差！（不要額外添加標點符號）"))["output"]
